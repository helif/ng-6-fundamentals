...
export class EventService {
    ...
    getServerEvents():Observable<IEvent[]> {
      return this.http.get<IEvent[]>('/api/events')
        .pipe(catchError(this.handleServerError<IEvent[]>('getServerEvents', [])));
    }

    getServerEvent(id:string):Observable<IEvent> {
      return this.http.get<IEvent>('/api/events/' + id)
        .pipe(catchError(this.handleServerError<IEvent>('getServerEvent')));
    }

    saveServerEvent(event):Observable<any> {
      let options = {headers: new HttpHeaders({'Content-Type':'application/json'})};
      return this.http.post<IEvent>('api/events', event, options).
        pipe(catchError(this.handleServerError<IEvent>('saveEvent')))
    }

    searchServerSessions(searchTerm:string):Observable<ISession[]> {
      return this.http.get<ISession[]>('/api/sessions/search?search=' + searchTerm)
        .pipe(catchError(this.handleServerError<ISession[]>('searchServerSessions', [])));
    }
    ...
    searchSessions(searchTerm:string):Observable<any[]> {
      let term:string = searchTerm.toLocaleLowerCase();
      var results = [];
...
const Events: IEvent[] = [
    ...
    //     country: 'USA'
    //   },
      onlineUrl: 'www.angularExcalibur.com',
      sessions: [
        {
          id: 1,
